...
public class OriOaiNuxeo2XmlServiceImpl extends DefaultComponent implements
        OriOaiNuxeo2XmlService {
    ...
    public void registerContribution(Object contribution,
        ...
        if (log.isDebugEnabled())
            log.debug("registerContribution :: " + "add " + contribution
                    + ", extensionPoint=" + extensionPoint + ", contributor="
                    + contributor);
    ...
    public void unregisterContribution(Object contribution,
            String extensionPoint, ComponentInstance contributor) {
        if (log.isDebugEnabled())
            log.debug("unregisterContribution :: " + "remove " + contribution
                    + ", extensionPoint=" + extensionPoint + ", contributor="
                    + contributor);
     ...
     *            URLs or just filenames. Path handling could be better, and
    ...
    private String getEmbeddedXmlMetadataFilePath(DocumentModel doc,
            Nuxeo2XmlDescriptor nuxeoDescriptor) throws ClientException {
                    ...
                    if (requestedBlob != null) {
                        ...
                        if (requestedFilename.endsWith(".zip")) {
                            try {
                                ...
                                if (log.isDebugEnabled())
                                    log.debug("getEmbeddedXmlMetadataFilePath :: versionUid="
                                            + versionUid);

                                String tempdir = System.getProperty("java.io.tmpdir");
                                if (log.isDebugEnabled())
                                    log.debug("getEmbeddedXmlMetadataFilePath :: tempdir="
                                            + tempdir);
                                File zipFile = new java.io.File(tempdir,
                                        "nuxeo-md-to-wf-" + versionUid + ".zip");
                                File zipDirectory = new java.io.File(tempdir,
                                        "nuxeo-md-to-wf-" + versionUid);
                                ...
                                if (files != null && files.length == 1
                                    ...
                                    if (log.isDebugEnabled())
                                        log.debug("getEmbeddedXmlMetadataFilePath :: one directory element :: "
                                                + rootDirecotoryName);
                                    zipDirectory = files[0];
                                }

                                // check if path exists
                                File embeddedMetadataFile = new File(
                                        zipDirectory,
                                        nuxeoDescriptor.getEmbeddedMetadataFilePath());
                                if (log.isDebugEnabled())
                                    log.debug("getEmbeddedXmlMetadataFilePath :: embeddedMetadataFile="
                                            + embeddedMetadataFile);
                                boolean exists = embeddedMetadataFile.exists();
                                if (log.isDebugEnabled())
                                    log.debug("getEmbeddedXmlMetadataFilePath :: exists="
                                            + exists);
                                ...
                                log.error(
                                        "getEmbeddedXmlMetadataFilePath :: problem unziping zip file from document :"
                                                + doc.getId(), ie);
                            }
                        }
                    }
    ...
    public String mergeXmlDoc(CoreSession documentManager, DocumentModel doc,
            String metadataSchemaNamespace, String workflowXmlDocument)
        ...
        try {
            if (log.isDebugEnabled()) {
                log.debug("mergeXmlDoc :: " + "documentRef : " + doc.getRef()
                        + " metadataSchemaNamespace : "
                        + metadataSchemaNamespace);
                log.debug("mergeXmlDoc :: nuxeo2XmlDescriptorMap = "
                        + nuxeo2XmlDescriptorMap);
                log.debug("mergeXmlDoc :: metadataSchemaNamespace = "
                        + metadataSchemaNamespace);
            }
            ...
            if (nuxeoDescriptor == null) {
                throw new ClientException(
                        metadataSchemaNamespace
                                + " not found in extension point nuxeoToxml of OriOaiNuxeo2XmlService. Probably in orioainuxeo2xml-contrib.xml");
            }
            if (log.isDebugEnabled())
                log.debug("mergeXmlDoc :: used nuxeoDescriptor = "
                        + nuxeoDescriptor);
            ...
            Document workflowRootDocument = workflowReader.read(new StringReader(
                    workflowXmlDocument));
            ...
            for (int n = 0; nuxeoDescriptorNamespaces != null
                    && n < nuxeoDescriptorNamespaces.length; n++) {
                Nuxeo2XmlDescriptorNamespace nuxeoDescriptorNamespace = nuxeoDescriptorNamespaces[n];
                nsWorkflowMap.put(nuxeoDescriptorNamespace.getPrefix(),
                        nuxeoDescriptorNamespace.getNamespace());
            }
            ...
            String embeddedXmlMetadataFilePath = getEmbeddedXmlMetadataFilePath(
                    doc, nuxeoDescriptor);
            if (embeddedXmlMetadataFilePath != null) {

                // get the root document element
                SAXReader embeddedReader = new SAXReader();
                Document embeddedRootDocument = embeddedReader.read(new File(
                        embeddedXmlMetadataFilePath));

                // merge
                workflowRootDocument = getMergedXmlDocs(
                        nuxeoDescriptorMetadatas,
                        Nuxeo2XmlDescriptorMetadata.TYPE_FILE,
                        embeddedRootDocument, nsWorkflowMap,
                        workflowRootDocument, nsWorkflowMap);

                if (log.isDebugEnabled())
                    log.debug("mergeXmlDoc :: embedded xml == "
                            + workflowRootDocument.asXML());
            }
            ...
            DocumentReader documentReader = new SingleDocumentReader(
                    documentManager, doc);
            String nuxeoXmlDocument = documentReader.read().getDocument().asXML();
            if (log.isDebugEnabled())
                log.debug("mergeXmlDoc :: nuxeoXmlDocument = "
                        + nuxeoXmlDocument);

            // namespaces
            HashMap<String, String> nsNuxeoMap = new HashMap<String, String>();
            nsNuxeoMap.put("dc", "http://purl.org/dc/elements/1.1/");
            nsNuxeoMap.put("oai_dc",
                    "http://www.openarchives.org/OAI/2.0/oai_dc/");
            nsNuxeoMap.put("nx_dc",
                    "http://www.nuxeo.org/ecm/schemas/dublincore/");
            nsNuxeoMap.put("nx_uid",
                    "http://project.nuxeo.com/geide/schemas/uid/");
            nsNuxeoMap.put("nx_common",
                    "http://www.nuxeo.org/ecm/schemas/common/");
            nsNuxeoMap.put("nx_file", "http://www.nuxeo.org/ecm/schemas/file/");
            nsNuxeoMap.put("nx_picture",
                    "http://www.nuxeo.org/ecm/schemas/picture");

            // get the root document element
            SAXReader nuxeoReader = new SAXReader();
            Document nuxeoRootDocument = nuxeoReader.read(new StringReader(
                    nuxeoXmlDocument));

            // merge
            workflowRootDocument = getMergedXmlDocs(nuxeoDescriptorMetadatas,
                    Nuxeo2XmlDescriptorMetadata.TYPE_NUXEO, nuxeoRootDocument,
                    nsNuxeoMap, workflowRootDocument, nsWorkflowMap);
            ...
            // merge
            workflowRootDocument = getMergedXmlDocs(nuxeoDescriptorMetadatas,
                    Nuxeo2XmlDescriptorMetadata.TYPE_STATIC, null, null,
                    workflowRootDocument, nsWorkflowMap);
            ...
            log.error("mergeXmlDoc :: IO error : documentRef : " + doc.getRef()
                    + " metadataSchemaNamespace : " + metadataSchemaNamespace,
                    e);
    ...
    public Document getMergedXmlDocs(
            Nuxeo2XmlDescriptorMetadata[] nuxeoDescriptorMetadatas,
            String type, Document fromRootDocument,
            HashMap<String, String> nsFromMap, Document workflowRootDocument,
            HashMap<String, String> nsWorkflowMap) throws JaxenException {
        ...
        for (Nuxeo2XmlDescriptorMetadata nuxeoDescriptorMetadata : nuxeoDescriptorMetadatas) {
            ...
            try {
                if (type.equals(nuxeoDescriptorMetadata.getType())) {
                    ...
                    if (fromXpath != null) {
                        if (log.isDebugEnabled())
                            log.debug("getMergedXmlDocs :: fromXpath="
                                    + fromXpath);
                        if (NUXEO_URL.equals(fromXpath)) {
                            fromXpath = nuxeoIdXpath;
                        }
                        Dom4jXPath fromXpathExpression = new Dom4jXPath(
                                fromXpath);
                        fromXpathExpression.setNamespaceContext(new SimpleNamespaceContext(
                                nsFromMap));
                        ...
                        if (fromElements != null) {
                            for (Object fromElement : fromElements) {
                                ...
                                if (NUXEO_URL.equals(nuxeoDescriptorMetadata.getFromXpath())) {
                                    elementValue = nuxeoUrlConfig.getDocumentUrl()
                                            + elementValue;
                                    if (log.isDebugEnabled())
                                        log.debug("getMergedXmlDocs :: nuxeoUrlConfig="
                                                + nuxeoUrlConfig);
                                }
                                if (log.isDebugEnabled())
                                    log.debug("getMergedXmlDocs :: elementValue="
                                            + elementValue);
                    ...
                    if (elementValues != null) {
                        ...
                        if (workflowXpathRoot != null) {
                            Dom4jXPath workflowXpathRootExpression = new Dom4jXPath(
                                    workflowXpathRoot);
                            if (log.isDebugEnabled())
                                log.debug("getMergedXmlDocs :: workflowXpathRoot="
                                        + workflowXpathRoot);
                            workflowXpathRootExpression.setNamespaceContext(new SimpleNamespaceContext(
                                    nsWorkflowMap));
                            Object workflowRootElementObject = workflowXpathRootExpression.selectSingleNode(workflowRootDocument);
                            if (log.isDebugEnabled())
                                log.debug("getMergedXmlDocs :: workflowRootElementObject="
                                        + workflowRootElementObject);

                            if (workflowRootElementObject == null) {
                                Node workflowRootNode = XmlTool.createNodeFromXpath(
                                        null, workflowXpathRoot,
                                        workflowRootDocument);
                                try {
                                    workflowRootDocument = new SAXReader().read(new StringReader(
                                            workflowRootNode.getDocument().asXML()));
                                } catch (Exception e) {
                                    e.printStackTrace();
                                ...
                                // workflowRootDocument =
                                // workflowRootNode.getDocument();
                            }
                            if (log.isDebugEnabled())
                                log.debug("getMergedXmlDocs :: workflowRootDocument="
                                        + workflowRootDocument.asXML());
                        }
                        ...
                        for (String elementValue : elementValues) {
                            ...
                            // translation
                            elementValue = translateValue(elementValue,
                                    translations);

                            // search element into workflow xml
                            Dom4jXPath workflowXpathExpression = new Dom4jXPath(
                                    workflowXpath);
                            workflowXpathExpression.setNamespaceContext(new SimpleNamespaceContext(
                                    nsWorkflowMap));
                            List workflowElementList = workflowXpathExpression.selectNodes(workflowRootDocument);

                            if (log.isDebugEnabled())
                                log.debug("getMergedXmlDocs :: workflowElementList="
                                        + workflowElementList);
                            ...
                            boolean empty = true;
                            for (int w = 0; !foundSameValue
                                    && workflowElementList != null
                                    && w < workflowElementList.size(); w++) {
                                ...
                                if (workflowElementObject instanceof Element) {
                                    ...
                                    if (log.isDebugEnabled())
                                        log.debug("getMergedXmlDocs :: workflowElementValue="
                                                + workflowElementValue);

                                    if (workflowElementValue.equals(elementValue)) {
                                        foundSameValue = true;
                                    }
                                    if (firstEmptyElementIndex == -1
                                            && workflowElementValue.equals("")) {
                                        firstEmptyElementIndex = w;
                                    }

                                    if (workflowElementValue != null
                                            && !workflowElementValue.equals("")) {
                                        empty = false;
                                    }
                            ...
                            if (!foundSameValue
                                ...
                                // we add a node
                                    ...
                                    // add the node
                                    if (log.isDebugEnabled()) {
                                        log.debug("workflowXpathRoot="
                                                + workflowXpathRoot);
                                        log.debug("workflowXpath="
                                                + workflowXpath);
                                    }
                                    // Node created =
                                    // XmlTool.createNodeFromXpath(workflowXpath,
                                    // workflowRootNode);
                                    Node created = XmlTool.createNodeFromXpath(
                                            workflowXpathRoot, workflowXpath,
                                            workflowRootDocument);

                                    if (nuxeoDescriptorMetadata.isCDATA()
                                            && created instanceof Element) {
