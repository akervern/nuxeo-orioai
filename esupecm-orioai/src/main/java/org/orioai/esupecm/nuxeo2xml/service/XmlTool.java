...
public class XmlTool {
    ...
    public static Node createNodeFromXpath(String rootXpath, String xpath,
            Document document) {
        ...
        if (lp != null) {
            ...
            if (!haveToCreate && steps.size() > 0) {
                ...
                String grandParentXpath = getXPathFromSteps(steps,
                        lp.isAbsolute());
                // System.out.println("grandParentXpath :: "+grandParentXpath);

                if (parentXpathRoot != null
                        && parentXpathRoot.equals(grandParentXpath)) {
                    haveToCreate = true;
                }
            }
            ...
            if (parent == null) {
                ...
                if (parent instanceof Element) {
                    ...
                    case Axis.ATTRIBUTE:
                        Attribute newAttribute = DocumentHelper.createAttribute(
                                null, name, "");
                    ...
                    log.error("createNodeFromXpath :: can not create child :: xpath="
                            + xpath);
                }
            }
        } else {
    ...
    private static void addPredicates(Element element, Predicate predicate) {
        ...
        if (predicateText.startsWith("(") && predicateText.endsWith(")")) {
            predicateText = predicateText.substring(1,
                    predicateText.length() - 1);
        }
        // System.out.println("   predicateText = "+predicateText);
        String[] predicateStepTexts = predicateText.split("/");
        for (int j = 0; predicateStepTexts != null
                && j < predicateStepTexts.length; j++) {
            String predicateStepText = predicateStepTexts[j];
            String predicateStepTextType = predicateStepText.substring(0,
                    predicateStepTexts[j].indexOf("::"));
            String predicateStepTextValue = predicateStepText.substring(predicateStepTextType.length()
                    + "::".length());
            ...
            if (predicateStepTextValue.indexOf("=") != -1) {
                predicateStepTextAttributeName = predicateStepTextValue.substring(
                        0, predicateStepTextValue.indexOf("=")).trim();
                predicateStepTextAttributeValue = predicateStepTextValue.substring(
                        predicateStepTextValue.indexOf("=") + 1).trim();
            ...
            if ("child".equals(predicateStepTextType)) {
                ...
                element = element.addAttribute(predicateStepTextAttributeName,
                        predicateStepTextAttributeValue);
            }
        }
    ...
    private static String getXPathFromSteps(List<Step> steps,
