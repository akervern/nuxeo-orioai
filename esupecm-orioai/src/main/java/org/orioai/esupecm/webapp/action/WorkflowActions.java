...
public class WorkflowActions implements Serializable {
    ...
    private OriOaiWorkflowService getOriOaiWorkflowService()
        ...
        if (oriOaiWorkflowService == null) {
            try {
                oriOaiWorkflowService = Framework.getService(OriOaiWorkflowService.class);
                if (oriOaiWorkflowService == null) {
                    fail = true;
                }
            } catch (Exception e) {
     ...
     * document
     * 
     * @param versionDoc a version document model
     * @return list of all ORI infos
     * @throws ClientException TODO : sort list by section
     */
    public List<OriOaiDocumentInfo> getVersionOriInfos(DocumentModel versionDoc)
            throws ClientException {

        if (useCache && oriInfosCache != null
                && oriInfosCache.containsKey(versionDoc.getId())) {
            if (log.isDebugEnabled())
                log.debug("getVersionOriInfos :: get from cache :: versionDoc.getId()="
                        + versionDoc.getId());
            ...
            if (log.isDebugEnabled())
                log.debug("getVersionOriInfos :: update cache :: versionDoc.getId()="
                        + versionDoc.getId() + ", oriIdProp=" + oriIdProp);
     ...
     * 
     * @param currentDoc a DocumentModel
     * @param oriIdProp a list of ORI ids
     * @return
     * @throws ClientException Note : store current idps to avoid time consuming
     *             WS requests
     */
    public List<OriOaiDocumentInfo> fetchOriInfosOld(DocumentModel currentDoc,
        ...
        if (oriIdProp != null) {
            for (String oriIdString : oriIdProp) {
                ...
                if (oriOaiWorkflowService != null) {
                    // the workflow idp
                    idp = oriOaiWorkflowService.getIdp(currentUser.getName(),
                            oriId);
                    if (idp != null) {
                        // the metadata type for this idp
                        metadataType = oriOaiWorkflowService.getMetadataType(
                                currentUser.getName(), idp);
                        if (metadataType != null) {

                            // user language
                            String language = FacesContext.getCurrentInstance().getViewRoot().getLocale().getLanguage();
                            if (log.isDebugEnabled())
                                log.debug("fetchOriInfos :: language="
                                        + language);

                            // the workflow states for this idp
                            Map<String, String> statesMap = oriOaiWorkflowService.getCurrentStates(
                                    currentUser.getName(), idp, language);
                            ...
                            informations = oriOaiWorkflowService.getCurrentInformations(
                                    currentUser.getName(), idp, language);
                            // the md editors to edit this XML medatata content
                            Map<String, String> mdEditorUrlsMap = oriOaiWorkflowService.getMdeditorUrls(
                                    currentUser.getName(), idp);
                            mdEditorUrls = getMdEditorInfos(mdEditorUrlsMap);
                            Map<String, String> actionsMap = oriOaiWorkflowService.getAvailableActions(
                                    currentUser.getName(), idp, language);
                            actions = getActions(actionsMap);
                        } else {
                            log.warn("fetchOriInfos :: metadataType is null for ORI ID "
                                    + oriId);
                            deletableRelation = true;
                        }
                    } else {
                        log.warn("fetchOriInfos :: idp is null for ORI ID "
                                + oriId);
                        deletableRelation = true;
                    }
                } else {
                ...
                if (!deletableRelation) {
                    OriOaiDocumentInfo newOriInfo = new OriOaiDocumentInfo(
                            oriId, metadataType, states, idp, mdEditorUrls,
                    ...
                    log.warn("fetchOriInfos :: delete relation for ORI ID "
                            + oriId);
                    try {
                        ...
                        facesMessages.add(
                                ...
                                resourcesAccessor.getMessages().get(
                                        "oriref.error.relation.deleted"),
                                params);
                    } catch (Exception e) {
                        log.error(
                                "fetchOriInfos :: can't delete relation for ORI ID "
                                        + oriId, e);
                    }
                }
            }
    ...
    protected OriOaiDocumentInfo getDocumentInfo(InstanceInfos infos) {
        ...
        List<ActionInfo> actions = null;
        // the metadata type for this idp
        metadataType = new OriOaiMetadataType(infos.getMetadataType(),
                infos.getMetadataTypeLabel());
        ...
        newOriInfo = new OriOaiDocumentInfo(infos.getOriId(), metadataType,
                states, infos.getIdp(), mdEditorUrls, actions, informations);
        ...
        if (oriIdProp == null) {
            ...
            for (String oriIdString : oriIdProp) {
                ...
                if (infos != null) {
                    ...
                    log.warn("fetchOriInfos :: instanceInfos is null for ORI ID "
                            + oriId);
                }
                if (deletableRelation) {
     ...
     *             WS requests
     */
    public List<OriOaiDocumentInfo> updateOriInfoCache(
            DocumentModel currentDoc, List<String> oriIdProp)
            throws ClientException {

        if (log.isDebugEnabled()) {
            log.debug("updateOriInfoCache :: currentDoc.getId()="
                    + currentDoc.getId());
            log.debug("updateOriInfoCache :: oriIdProp=" + oriIdProp);
        }

        /*
         * try { String s = null; s.substring(2, 3); } catch(Exception e) {
        ...
        if (oriInfosCache.containsKey(currentDoc.getId())) {
            if (log.isDebugEnabled())
                log.debug("updateOriInfoCache :: oriInfosCache contains "
                        + currentDoc.getId());
            ...
            for (int old = 0; oldInfos != null && old < oldInfos.size(); old++) {
                ...
                boolean found = false;
                for (int up = 0; !found && oriInfos != null
                        && up < oriInfos.size(); up++) {
                ...
                if (!found) {
                    if (log.isDebugEnabled())
                        log.debug("updateOriInfoCache :: add to oriInfos oldInfo="
                                + oldInfo);
            ...
            if (log.isDebugEnabled())
                log.debug("updateOriInfoCache :: oriInfosCache doesn't contain "
                        + currentDoc.getId());
        }

        if (log.isDebugEnabled())
            log.debug("updateOriInfoCache :: put to oriInfosCache currentDoc.getId()="
                    + currentDoc.getId() + ", oriInfos=" + oriInfos);
     ...
     * 
     * @param actionsMap contains id/name entries
     ...
     * @throws ClientException
     */
    public String processWorkflowAction(DocumentModel versionDoc)
            throws ClientException {

        if (log.isDebugEnabled())
            log.debug("processWorkflowAction :: actionNameParam="
                    + actionNameParam + ", oriIdParam=" + oriIdParam);
        ...
        if (actionIdParam != null && oriIdParam != null) {
            ...
            String observation = resourcesAccessor.getMessages().get(
                    "oriref.action.performed.observation");
            if (log.isDebugEnabled())
                log.debug("processWorkflowAction :: observation=" + observation);

            boolean performActionResult = oriOaiWorkflowService.performAction(
                    currentUser.getName(), oriIdpParam, actionId, observation);
            ...
            if (performActionResult) {
                String msg = resourcesAccessor.getMessages().get(
                        "oriref.action.performed.success");
                facesMessages.add(FacesMessage.SEVERITY_INFO, msg
                        + actionNameParam);
                if (log.isDebugEnabled())
                    log.debug("processWorkflowAction :: performActionResult="
                            + performActionResult + " --> add message=" + msg
                            + actionNameParam);
                ...
                String msg = resourcesAccessor.getMessages().get(
                        "oriref.action.performed.failure");
     ...
     * @throws ClientException
     */
    public String initOriWorkflow(DocumentModel versionToRef,
            String selectedMetadataType, String callEditor)
            throws ClientException {

        if (log.isDebugEnabled())
            log.debug("initOriWorkflow :: selectedMetatypes="
                    + selectedMetadataType + ", callEditor=" + callEditor);
        ...
        /* String createRelationsMessage = */createNewReferencedRelations(
                versionToRef, oriIdsMap);
        // facesMessages.add(FacesMessage.SEVERITY_INFO,
        // createRelationsMessage);

        // update oriinfo cache
        updateOriInfoCache(versionToRef,
                new ArrayList<String>(oriIdsMap.keySet()));

        Object[] params = new Object[2];
        params[0] = versionToRef.getName();
        params[1] = getMetadataType(selectedMetadataType).getLabel();
        facesMessages.add(FacesMessage.SEVERITY_INFO,
                resourcesAccessor.getMessages().get("oriref.report"), params);
     ...
     * @param versionDocu
     * @param versionModel
     * @param orIdsToRef a map of ( ori Id / metadata type label) pairs
     * @return
     * @throws ClientException
     */
    protected String createNewReferencedRelations(DocumentModel versionDocu,
            Map<String, String> orIdsToRef) throws ClientException {
        if (log.isDebugEnabled())
            log.debug("initOriWorkflow :: set the property oriId and create a version"
                    + "proxyDoc.ref : "
                    + versionDocu.getRef()
                    + " oriIdsToRef : " + orIdsToRef.toString());
        String refMessage = "";

        try {
            // 2. create a relation on each id
            for (Map.Entry<String, String> oriIdToRef : orIdsToRef.entrySet()) {
                String mesgStatus = referencingManager.createOriRelation(
                        oriIdToRef.getKey(), versionDocu, oriIdToRef.getValue());
                refMessage += "\n"
            ...
            log.error(
                    "createNewReferencedRelations :: can't create relation for referencing",
                    e);
        }
     ...
     * 
     * @param proxyDoc : proxy of the version from which document will be
     *            cloned, oriIds added
     * @param type
    ...
    protected Long initWorkflow(DocumentModel proxyDoc, String type)
        ...
        log.info("initWorkflow :: try to init a new ori-oai workflow (metadataTypeId="
                + metadataTypeId + ") ...");
        ...
        try {
            ...
            // ID
            Long oriWorkflowId = oriOaiWorkflowService.newWorkflowInstance(
                    currentUser.getName(), metadataTypeId);
            if (log.isDebugEnabled())
                log.debug("initWorkflow :: metadataTypeId=" + metadataTypeId
                        + ", currentUser.getName()=" + currentUser.getName()
                        + " --> oriWorkflowId=" + oriWorkflowId);

            // get the idp
            String idp = oriOaiWorkflowService.getIdp(currentUser.getName(),
                    oriWorkflowId);
            if (log.isDebugEnabled())
                log.debug("initWorkflow :: idp=" + idp);

            // get the original XML from the workflow
            String xmlFromWorkflow = oriOaiWorkflowService.getXMLForms(
                    currentUser.getName(), idp);
            ...
            String metadataSchemaNamespace = oriOaiWorkflowService.getMetadataSchemaNamespace(
                    currentUser.getName(), metadataTypeId);

            // create XML metadata content
            String mergedXmlContent = oriOaiNuxeo2XmlService.mergeXmlDoc(
                    documentManager, proxyDoc, metadataSchemaNamespace,
                    xmlFromWorkflow);
            if (log.isDebugEnabled())
                log.debug("initWorkflow :: mergedXmlContent="
                        + mergedXmlContent);

            // return this XML to workflow
            oriOaiWorkflowService.saveXML(currentUser.getName(), idp,
                    mergedXmlContent);
            ...
            processCallEditor(idp, oriWorkflowId, proxyDoc,
                    getMdEditorInfos(mdEditorUrlsMap));

            /*
             * for (int i=0; currentMdEditorUrls!=null &&
             * i<currentMdEditorUrls.size(); i++) { String url =
             * currentMdEditorUrls.get(i).getUrl(); url = url + "?idp=" + idp;
             * currentMdEditorUrls.get(i).setUrl(url); }
             */
     ...
     * @throws ClientException
     */
    public Collection<ReferencingVersion> getVersionsReferencedModel()
            throws ClientException {
        ...
        DocumentModel doc = navigationContext.getCurrentDocument();
        if (log.isDebugEnabled()) {
            log.debug("getVersionsReferencedModel :: doc.getId()="
                    + doc.getId());
        ...
        for (VersionModel versionModel : versions) {
            if (log.isDebugEnabled()) {
                ...
                log.debug("getVersionsReferencedModel :: doc.getRef()="
                        + doc.getRef());
            }

            DocumentModel versionDoc = documentManager.getDocumentWithVersion(
                    doc.getRef(), versionModel);
            if (log.isDebugEnabled())
                log.debug("getVersionsReferencedModel :: versionDoc.getId()="
                        + versionDoc.getId());
            ...
            if (versionDoc != null) {
                ...
                if (log.isDebugEnabled())
                    log.debug("getVersionsReferencedModel :: versionLabel="
                            + versionLabel);
                ...
                if (!versionsProxiesModelMap.containsKey(versionDoc.getId())) {
                    ...
                    if (log.isDebugEnabled())
                        log.debug("getVersionsReferencedModel :: allProxyOriInfos="
                                + allProxyOriInfos);
                    ReferencingVersion referencingVersion = new ReferencingVersion(
                            versionLabel, versionDoc, versionModel,
                            allProxyOriInfos);
                    if (log.isDebugEnabled())
                        log.debug("getVersionsReferencedModel :: versionDoc.getId()="
                                + versionDoc.getId());
                    versionsProxiesModelMap.put(versionDoc.getId(),
                            referencingVersion);
                }
            }
            log.debug("getVersionsReferencedModel");
        ...
        List<ReferencingVersion> versionsList = new ArrayList<ReferencingVersion>(
                result);
    ...
    public List<OriOaiMetadataType> getReferencableMetadataTypes(
        ...
        if (log.isDebugEnabled())
            log.debug("getReferencableMetadataTypes :: selectedProxyOriInfos="
                    + selectedProxyOriInfos);
        List<OriOaiMetadataType> availTypes = new ArrayList<OriOaiMetadataType>(
                getMetadataTypes());
        if (log.isDebugEnabled())
            log.debug("getReferencableMetadataTypes :: availTypes="
                    + availTypes);
        for (OriOaiDocumentInfo oriInfo : selectedProxyOriInfos) {
            // doesn't work (bad comparator) :
            // availTypes.remove(oriInfo.getMetadataType());
     ...
     * @throws ClientException
     */
    public Map<String, String> getReferencableMetadataTypesMap(
            DocumentModel versionDoc) throws ClientException {
        ...
        if (log.isDebugEnabled())
            log.debug("getReferencableMetadataTypesMap :: metadataTypes="
                    + metadataTypes);

        Map<String, String> metadataTypesMap = new HashMap<String, String>(
                metadataTypes.size());
        ...
        if (log.isDebugEnabled())
            log.debug("getReferencableMetadataTypesMap :: metadataTypesMap="
                    + metadataTypesMap);
    ...
    public List<OriOaiMetadataType> getMetadataTypes() throws ClientException {
        ...
        if (log.isDebugEnabled())
            log.debug("getMetadataTypes :: 3 currentUser.getName()="
                    + currentUser.getName());

        List<OriOaiMetadataType> userMetadataTypes = metadataTypes.get(currentUser.getName());

        if (log.isDebugEnabled())
            log.debug("getMetadataTypes :: 4 userMetadataTypes="
                    + userMetadataTypes);

        if (userMetadataTypes == null) {
            userMetadataTypes = getOriOaiWorkflowService().getMetadataTypes(
                    currentUser.getName());
            metadataTypes.put(currentUser.getName(), userMetadataTypes);
        }

        if (log.isDebugEnabled())
            log.debug("getMetadataTypes :: 5 userMetadataTypes="
                    + userMetadataTypes);
    ...
    public String processCallEditor(String idp, Long id,
            DocumentModel versionDoc, EditorInfo mdEditorUrl) {
    ...
    public String processCallEditor(String idp, Long id,
            DocumentModel versionDoc, List<EditorInfo> urls) {
        ...
        if (log.isDebugEnabled())
            log.debug("processCallEditor :: this.callEditor=" + this.callEditor
                    + "; this.currentMdEditorId=" + this.currentMdEditorId
                    + "; this.currentMdEditorIdp=" + this.currentMdEditorIdp);
        return null;
    }

    public boolean getCallEditor() {
        if (log.isDebugEnabled())
            log.debug("getCallEditor :: this.callEditor=" + this.callEditor
                    + "; this.currentMdEditorId=" + this.currentMdEditorId
                    + "; this.currentMdEditorIdp=" + this.currentMdEditorIdp);
        if (this.callEditor) {
    ...
    public List<EditorInfo> getCurrentMdEditorUrls() throws ClientException {
        if (this.currentMdEditorUrls != null) {
            if (log.isDebugEnabled())
                log.debug("getCurrentMdEditorUrls :: currentMdEditorUrls is not null :: currentMdEditorUrls="
                        + this.currentMdEditorUrls);
        } else {
            ...
            // get the md editor url
            if (log.isDebugEnabled()) {
                log.debug("getCurrentMdEditorUrls :: this.currentMdEditorId="
                        + this.currentMdEditorId);
                log.debug("getCurrentMdEditorUrls :: this.currentMdEditorIdp="
                        + this.currentMdEditorIdp);
            }
            Map<String, String> mdEditorUrlsMap = oriOaiWorkflowService.getMdeditorUrls(
                    currentUser.getName(), this.currentMdEditorIdp);
            if (log.isDebugEnabled())
                log.debug("getCurrentMdEditorUrls :: mdEditorUrlsMap="
                        + mdEditorUrlsMap);
    ...
    public String getCurrentMdEditorIdp() {
        if (log.isDebugEnabled())
            log.debug("getCurrentMdEditorIdp :: this.currentMdEditorIdp="
                    + this.currentMdEditorIdp);
    ...
    public Long getCurrentMdEditorId() {
        if (log.isDebugEnabled())
            log.debug("getCurrentMdEditorId :: this.currentMdEditorId="
                    + this.currentMdEditorId);
    ...
    public DocumentModel getCurrentMdEditorVersionDoc() {
        if (log.isDebugEnabled())
            log.debug("getCurrentMdEditorVersionDoc :: this.currentMdEditorVersionDoc="
                    + this.currentMdEditorVersionDoc);
        return this.currentMdEditorVersionDoc;
    }

    public String backFromEditor() throws ClientException {
        if (log.isDebugEnabled())
            log.debug("backFromEditor :: currentMdEditorId="
                    + currentMdEditorId);
        ...
        if (log.isDebugEnabled()) {
            ...
            log.debug("backFromEditor :: currentMdEditorVersionDoc="
                    + currentMdEditorVersionDoc);
        }
