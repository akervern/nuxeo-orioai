...
public class OriOaiWorkflowServiceImpl extends DefaultComponent implements
    ...
    private IOriWorkflowService getRemoteOriWorkflowService(String username) {
        ...
        if (oriWorkflowService == null) {
            ...
            log.info("getRemoteOriWorkflowService :: contacting Web Service from URL : "
                    + wsUrl);

            Service serviceModel = new ObjectServiceFactory().create(IOriWorkflowService.class);

            try {
                oriWorkflowService = (IOriWorkflowService) new XFireProxyFactory().create(
                        serviceModel, wsUrl);
                _oriWorkflowServices.put(username, oriWorkflowService);
            } catch (MalformedURLException e) {
                throw new RuntimeException(
                        "pb retireving ori-oai-workflow Web Service", e);
            }
        }
    ...
    public List<OriOaiMetadataType> getMetadataTypes(String username) {
        ...
        if (log.isDebugEnabled())
            log.debug("getMetadataTypes :: going to get metadataTypes for "
                    + username);
        ...
        if (log.isDebugEnabled())
            log.debug("getMetadataTypes :: get metadataTypes for " + username
                    + " from Web Service ok : " + metadataTypes.toString());
        ...
        while (metadataTypeIds.hasNext()) {
            ...
            OriOaiMetadataType metadataType = new OriOaiMetadataType(
                    metadataTypeId, metadataTypeLabel);
    ...
    public Long newWorkflowInstance(String username, String metadataTypeId) {
        ...
        Long id = oriWorkflowService.newWorkflowInstance(null, metadataTypeId,
                username);
    ...
    /**
     * @deprecated use Vector<String> getCurrentStates(Map<String, String>
     *             statesMap) instead
     */
    public Map<String, String> getCurrentStates(String username, String idp,
            String language) {
        IOriWorkflowService oriWorkflowService = getRemoteOriWorkflowService(username);

        Map<String, String> currentStates = oriWorkflowService.getCurrentStates(
                idp, language);
        if (log.isDebugEnabled())
    ...
    /**
     * @deprecated use List<String> getCurrentInformations(Map<String, String>
     *             currentInformations) instead
     */
    public List<String> getCurrentInformations(String username, String idp,
            String language) {
        ...
        Map<String, String> currentInformations = oriWorkflowService.getErrors(
                idp, language);
        if (log.isDebugEnabled())
            log.debug("getCurrentInformations :: currentInformations="
                    + currentInformations);
    ...
    public OriOaiMetadataType getMetadataType(String username, String idp) {
        ...
        try {
            ...
            return new OriOaiMetadataType(metadataTypeId,
                    oriWorkflowService.getMetadataTypes().get(metadataTypeId));
        } catch (Exception e) {
            log.error(
    ...
    public String getMetadataSchemaNamespace(String username,
        ...
        try {
            ...
            // String metadataSchemaNamespace =
            // "http://www.abes.fr/abes/documents/tef";
            return metadataSchemaNamespace;
        } catch (Exception e) {
            log.error(
                    "getMetadataSchemaNamespace :: can't retrieve metadata namespace from metadataTypeId "
                            + metadataTypeId, e);
            return null;
        }
    }
    ...
    private String getReplacedMdEditorUrl(String mdEditorUrl) {
        if (log.isDebugEnabled()) {
            ...
            log.debug("getReplacedMdEditorUrl :: config.getMdEditorFromUrl()="
                    + config.getMdEditorFromUrl());
            log.debug("getReplacedMdEditorUrl :: config.getMdEditorToUrl()="
                    + config.getMdEditorToUrl());
            log.debug("getReplacedMdEditorUrl :: config.isMdEditorTranslationSet()="
                    + config.isMdEditorTranslationSet());
        }

        String result = mdEditorUrl;

        if (config.isMdEditorTranslationSet()) {
            result = mdEditorUrl.replaceFirst(config.getMdEditorFromUrl(),
                    config.getMdEditorToUrl());
        }
     ...
     * @return
     */
    public Map<String, String> getAvailableActions(String username, String idp,
            String language) {
        ...
        IOriWorkflowService oriWorkflowService = getRemoteOriWorkflowService(username);
        Map<String, String> actionsMap = oriWorkflowService.getAvailableActions(
                idp, language);
     ...
     */
    public boolean performAction(String username, String idp, int actionId,
            String observation) {
        log.info("performAction :: idp=" + idp + ", actionId=" + actionId
