...
@Scope(CONVERSATION)
public class OriOaiRelationActionsBean extends CustomRelationActions implements
        OriOaiRelationActions {
     ...
     */
    public String createOriRelation(String oriId, DocumentModel docToRefer,
            String metadataTypeLabel) throws ClientException {
     ...
     * @throws ClientException
     */
    public void deleteOriRelation(Long oriId, DocumentModel document)
            throws ClientException {
        resetStatements();
        ...
        if (statementInfos != null && statementInfos.size() > 0) {
            ...
            for (StatementInfo info : statementInfos) {
                ...
                if (object != null && object instanceof LiteralImpl) {
                    ...
                    if (log.isDebugEnabled())
                        log.debug("removeOriRelation :: oriIds=" + oriIds
                                + ", objectValue=" + objectValue);
                    if (oriIds.equals(objectValue)) {
                        if (log.isDebugEnabled())
                            log.debug("removeOriRelation :: EQUALS oriId="
                                    + oriId);
    ...
    public List<String> getOriIdsForDocument(DocumentModel document)
            throws ClientException {
        resetStatements();
        List<StatementInfo> statementInfos = getOutgoingStatementsInfo(document);
        if (log.isDebugEnabled())
            log.debug("getOriIdsForDocument :: statementInfos="
                    + statementInfos);
